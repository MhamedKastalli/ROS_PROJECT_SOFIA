...
{	ros::init(argc, argv, "process_pc");
	...
	pcl_conversions::toPCL(point, *cloud_pass);
	pcl::PassThrough<pcl::PCLPointCloud2> filter_z;
	double min_range = -1.0;
	double max_range = 1.05;	
	filter_z.setInputCloud(cloud_pass);
	filter_z.setFilterFieldName("y");
	filter_z.setFilterLimits(min_range, max_range);
	filter_z.filter(*filtered);
